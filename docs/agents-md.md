---
description: "The AGENTS.md from potato-skill: what Codex, Cursor and Claude Code follow to build a Potato annotation task, with no install step."
---
...
Claude Code, Codex and Cursor all read `AGENTS.md`, so it needs no install step. Copy it into the project you are working in. The text below is that file.

---
description: "The AGENTS.md from potato-skill: short instructions for building a Potato annotation task that Codex and Cursor read from the project root."
---

<!-- Generated by scripts/generate_references.py from AGENTS.md. Do not edit by hand. -->

# Agent Instructions

Codex and Cursor read `AGENTS.md` from the project root, so it needs no install step: copy it into the project you are working in. Claude Code reads `CLAUDE.md` instead, and a `CLAUDE.md` holding the line `@AGENTS.md` imports it.

It is the short version. The references and helper scripts it points to come with the skill, installed as below. The text after the rule is the file itself.
...
9 executable helpers -- and `dist/potato-skill.zip`. One build, three destinations:
